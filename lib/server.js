...
const start = function (port) {
    ...
    const server = net.createServer((socket) => {
        ...
        socket.on("data", (data) => {
            ...
            if (bufferData) {
                try {
                    context = JSON.parse(bufferData + context);
                } catch (error) {
                    console.log("大buffer异常", error);
                }
                bufferData = "";
            } else {
                try {
            ...
            if (context.type == "register") {
                ...
                // 回复请求
                // socket 会截断中文，导致乱码，所以需要删除header里的content-length
                // reqClient[context.id].write(context.data);
                for (const data of context.data.split("\r\n")) {
                    let isHeader = true;
                    if (!isHeader || data) {
                        if (!data.startsWith("Content-Length")) {
                            reqClient[context.id].write(data + "\r\n");
                        }
                    } else {
                        isHeader = false;
                        reqClient[context.id].write("\r\n");
                    }
                }
